...
def process_country(iso3, normalised_data, deltap_dataset):
    ...
    combined_data = country_masked_normalised * deltap_masked
    pixel_count = np.sum(~np.isnan(combined_data))
    mean_value = np.nanmean(combined_data)
    mean_err = np.nanstd(combined_data) / np.sqrt(np.sum(~np.isnan(combined_data))) 
    return mean_value, mean_err, pixel_count
...
for year in years:
    ...
    with tqdm(total=total_items, desc=f"Calculating delta-p for crops in {year} for {len(country_isos)} countries", unit="item") as pbar:
        ...
        for band_idx in range(1, band_count + 1):
            ...
            sp_totals = pd.read_csv(os.path.join("data", "data_dirs", year, "deltap", "restore_agriculture", "0.25", "totals.csv"))
            ...
            for item_name, item_index in spam_data.items():
                ...
                for iso3 in country_isos: 
                    mean_value, mean_err, pixel_count = process_country(iso3, normalised_data, band_data)

                    output_data.loc[len(output_data), ["ISO3", "item_name", "band_name", "deltap_mean", "deltap_mean_sem", "unit", "pixel_count", "sp_count"]] = [
                         iso3, item_name, band_name, mean_value, mean_err, "deltaP per km2 per sp.", pixel_count, sp_count]
